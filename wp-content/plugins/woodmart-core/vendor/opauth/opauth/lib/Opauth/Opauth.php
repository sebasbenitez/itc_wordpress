...
class Opauth {
	...
	public function __construct($config = array(), $run = true) {
		...
		if ($this->env['security_salt'] == 'LDFmiilYf8Fyw5W10rx4W1KsVrieQCnpBzzpTBWA5vJidQKDx8pMJbmw28R1C4m'){
			wp_trigger_error( __METHOD__, 'Please change the value of \'security_salt\' to a salt value specific to your application', E_USER_NOTICE);
		}
	...
	public function run() {
		...
		if (!empty($this->env['params']['strategy'])) {
			if (strtolower($this->env['params']['strategy']) == 'callback') {
				...
				wp_trigger_error( __METHOD__, 'Unsupported or undefined Opauth strategy - '.$this->env['params']['strategy'], E_USER_ERROR);
			}
		} else {
			$sampleStrategy = array_pop($this->env['Strategy']);
			wp_trigger_error( __METHOD__, 'No strategy is requested. Try going to '.$this->env['complete_path'].$sampleStrategy['strategy_url_name'].' to authenticate with '.$sampleStrategy['strategy_name'], E_USER_NOTICE);
		}
	}
	...
	private function loadStrategies() {
		if (isset($this->env['Strategy']) && is_array($this->env['Strategy']) && count($this->env['Strategy']) > 0) {
			...
			wp_trigger_error( __METHOD__, 'No Opauth strategies defined', E_USER_ERROR);
		}
	}
	...
	private function requireStrategy($strategy) {
		if (!class_exists($strategy.'Strategy')) {
			...
			if (!$found) {
				wp_trigger_error( __METHOD__, 'Strategy class file ('.$this->env['strategy_dir'].$strategy.'/'.$strategy.'Strategy.php'.') is missing', E_USER_ERROR);
			}
		}
