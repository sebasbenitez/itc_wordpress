...
namespace WOODMART_CORE\Importer;

use WP_Importer;
use WP_Error;
use WOODMART_CORE\Importer\Parser\WXR_Parser;
...
class Import extends WP_Importer {
	/**
	 * Maximum WXR version supported by this importer.
	 *
	 * @var float
	 */
	...
	/**
	 * ID of the attachment post for the uploaded WXR file
	 *
	 * @var int
	 */
	public $id;
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * Version of the WXR file being imported, e.g. 1.0, 1.1, 1.2
	 *
	 * @var string
	 */
	public $version;
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save authors information.
	 *
	 * @var array
	 */
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save posts information.
	 *
	 * @var array
	 */
	public $posts = array();
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save terms information.
	 *
	 * @var array
	 */
	public $terms = array();
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save categories information.
	 *
	 * @var array
	 */
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save tags information.
	 *
	 * @var array
	 */
	public $tags = array();
	...
	/**
	 * Data parsed from the WXR file, used for processing and importing.
	 * This atribute for save base URL of the source site information.
	 *
	 * @var string
	 */
	public $base_url = '';
	...
	/**
	 * Stores import mapping and metadata required after the core import pass.
	 *
	 * Used to keep old-to-new IDs for posts and terms, including menu items,
	 * and extra flags (for example, imported builder data type). At the end
	 * of import this structure is persisted to wd_imported_data_{version}.
	 *
	 * @var array
	 */
	public $wd_data = array();

	/**
	 * Whether to download attachments using the CDN URL if the original source URL is not accessible.
	 *
	 * @var bool
	 */
	public $wd_download_by_cdn = false;

	/**
	 * Mapping of source author IDs to local user IDs created or assigned during import.
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of source author logins to local user IDs.
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of source term IDs to local term IDs for all taxonomies.
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of source post IDs to local post IDs.
	 *
	 * @var array
	 */
	...
	/**
	 * Stores posts whose parent post has not been imported yet.
	 *
	 * Key is source child post ID, value is source parent post ID.
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of source nav menu item post IDs to local nav menu item IDs.
	 *
	 * @var array
	 */
	...
	/**
	 * Stores menu items whose parent menu item has not been imported yet.
	 *
	 * Key is source child menu item ID, value is source parent menu item ID.
	 *
	 * @var array
	 */
	...
	/**
	 * Menu items deferred because the referenced object is not available yet.
	 *
	 * @var array
	 */
	...
	/**
	 * Whether to fetch attachments during import. Set in import options and used during post processing to determine whether to fetch attachments.
	 *
	 * @var bool
	 */
	...
	/**
	 * Mapping of old attachment URLs to new attachment URLs for imported attachments. Used for backfilling attachment URLs after import.
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of old attachment IDs to new attachment IDs for imported attachments. Used for remapping featured images after import.
	 *
	 * @var array
	 */
	 ...
	 * @param string|false $version Version of the WXR file being imported, e.g. 1.0, 1.1, 1.2. Optional, used for storing import metadata in wd_imported_data_{version}.
	 */
	public function import( $file, $version = false ) {
	...
	public function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			printf(
				wp_kses_post(
					// translators: %s is the path to the uploaded WXR file that could not be found.
		...
		$this->version = $import_data['version'];
		if ( (float) $this->version > (float) $this->max_wxr_version ) {
			echo '<div class="error"><p><strong>';
			printf(
				// translators: %s is the version of the uploaded WXR file that is not supported by this importer.
	...
	public function get_authors_from_import( $import_data ) {
		if ( ! empty( $import_data['authors'] ) ) {
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf(
						// translators: %s is the author name from the WXR file that could not be imported.
			...
			<?php if ( ! empty( $this->authors ) ) : ?>
				...
				<?php if ( $this->allow_create_users() ) : ?>
					<p>
						<?php
						printf(
							// translators: %s is the default user role for new users created during import, as set in WordPress settings.
	...
	public function author_select( $n, $author ) {
		...
		if ( '1.0' != $this->version ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
		...
		if ( $create_users ) {
			echo '<label for="user_new_' . absint( $n ) . '">';
			if ( '1.0' != $this->version ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
			...
			echo ' <input type="text" id="user_new_' . absint( $n ) . '" name="user_new[' . absint( $n ) . ']" value="' . esc_attr( $value ) . '" /><br />';
		}

		echo '<label for="imported_authors_' . absint( $n ) . '">';
		if ( ! $create_users && '1.0' == $this->version ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
		...
		echo ' ' . wp_dropdown_users(
			array(
				'name'            => 'user_map[' . absint( $n ) . ']',
				'id'              => 'imported_authors_' . absint( $n ),
				'multi'           => true,
		...
		echo '<input type="hidden" name="imported_authors[' . absint( $n ) . ']" value="' . esc_attr( $author['author_login'] ) . '" />';
	...
	public function get_author_mapping() {
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) { // phpcs:ignore WordPress.Security
			...
			if ( ! empty( $_POST['user_map'][ $i ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				...
				if ( ! empty( $_POST['user_new'][ $i ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
					$user_id = wp_create_user( $_POST['user_new'][ $i ], wp_generate_password() ); // phpcs:ignore WordPress.Security
				} elseif ( '1.0' != $this->version ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
					$user_data = array(
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					// translators: %s is the author display name from the WXR file that could not be imported as a new user.
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				printf(
					// translators: %s is the category nicename from the WXR file that could not be imported.
					esc_html__( 'Failed to import category %s', 'wordpress-importer' ),
	...
	public function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf(
					// translators: %s is the tag name from the WXR file that could not be imported.
					esc_html__( 'Failed to import post tag %s', 'wordpress-importer' ),
	...
	public function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf(
					// translators: 1 is the term taxonomy, 2 is the term name, both from the WXR file that could not be imported.
	...
	public function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf(
					// translators: 1 is the post title, 2 is the post type, both from the WXR file that could not be imported.
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
				...
				if ( is_wp_error( $post_id ) ) {
					printf(
						// translators: 1 is the post type, 2 is the post title, both from the WXR file that could not be imported.
						esc_html__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'wordpress-importer' ),
				...
				if ( 1 == $post['is_sticky'] ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
					stick_post( $post_id );
				}
			}
	...
	public function process_menu_item( $item ) {
		...
		if ( ! $menu_id ) {
			printf(
				// translators: %s is the menu slug from the WXR file that could not be found in the site.
	...
	public function fetch_remote_file( $url, $post ) {
		...
		// XTemos.
		$safe_remote_iterations = apply_filters( 'woodmart_safe_remote_iterations', 3 );

		for ( $i = 1; $i <= $safe_remote_iterations; $i++ ) {
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					// translators: %s is the maximum file size allowed, as returned by the max_attachment_size() method.
	...
	public function backfill_parents() {
		...
		foreach ( $this->post_orphans as $child_id => $parent_id ) {
			...
			if ( $local_child_id && $local_parent_id ) {
				$wpdb->update( $wpdb->posts, array( 'post_parent' => $local_parent_id ), array( 'ID' => $local_child_id ), '%d', '%d' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	...
	public function backfill_attachment_urls() {
		...
		foreach ( $this->url_remap as $from_url => $to_url ) {
			...
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s)", $from_url, $to_url ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			// remap enclosure urls
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure'", $from_url, $to_url ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		}
	}
	...
	public function header() {
		...
		if ( isset( $updates[ $basename ] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf(
				// translators: %s is the new version number of the WordPress Importer plugin that is available for update.
	...
	/**
	 * Close divs opened in header() and display import page footer.
	 */
	 ...
	 *
	 * @param int $val The default timeout value, in seconds.
	 *
	 * @return int 60
	 */
	 ...
	 *
	 * @return int Negative if $a is longer, positive if $b is longer, 0 if they are the same length.
	 */
