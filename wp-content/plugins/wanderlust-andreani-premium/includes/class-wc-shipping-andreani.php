...
class WC_Shipping_Andreani extends WC_Shipping_Method {
	...
	public function init() {
		...
		$this->origin_telefono			 = $this->get_option( 'origin_telefono' );
		$this->origin_dni			       = $this->get_option( 'origin_dni' );
	...
	*/
		...
		$service_preciofijo     = isset( $_POST['service_preciofijo'] ) ? $_POST['service_preciofijo'] : array();
			...
			for ( $i = 0; $i <= max( array_keys( $service_operativa ) ); $i ++ ) {
				...
				if ( $service_operativa[ $i ] ) {
  					$services[] = array(
						...
						'preciofijo'     => floatval( $service_preciofijo[ $i ] ),
	...
	public function calculate_shipping( $package = array() ) {
		...
		$andreani_package = $andreani_packages[0]['GroupPackageCount'];	 $dimension_unit = esc_attr( get_option('woocommerce_dimension_unit' )); $weight_unit = esc_attr( get_option('woocommerce_weight_unit' )); $weight_multi = 0; $dimension_multi = 0;
 		if ($dimension_unit == 'm') { $dimension_multi =  1;} if ($dimension_unit == 'cm') {  $dimension_multi =  100;} if ($dimension_unit == 'mm') { $dimension_multi =  1000;} if ($weight_unit == 'kg') { $weight_multi =  1;} if ($weight_unit == 'g') {  $weight_multi =  0.001;}
		...
		$origen_datos[] = array ( 'origin' => $this->origin, 'api_key' => $this->api_key, 'origin_contacto' => $this->origin_contacto, 'dni_destino' => $this->destino_dni, 'origin_email' => $this->origin_email, 'origin_telefono' => $this->origin_telefono, 'origin_dni' => $this->origin_dni, 'origin_calle' => $this->origin_calle, 'origin_numero' => $this->origin_numero, 'origin_piso' => $this->origin_piso, 'origin_depto' => $this->origin_depto, 'origin_localidad' => $this->origin_localidad, 'origin_provincia' => $this->origin_provincia, 'origin_observaciones' => $this->origin_observaciones, 'api_user' => $this->api_user, 'api_password' => $this->api_password, 'api_nrocuenta' => $this->api_nrocuenta, 'api_confirmarretiro' => $this->api_confirmarretiro,	 'andreani_lenth' => $andreani_lenth, 'andreani_width' => $andreani_width, 'andreani_height' => $andreani_height, 'andreani_amount' => $andreani_amount, 'andreani_weightb' => $andreani_weightb, );
		$origen_datos = json_encode($origen_datos); $_SESSION['origen_datos'] = $origen_datos;   setcookie('andreani_origen_datos', $origen_datos, time() + (86400 * 30), "/"); $seguro = round($package['contents_cost']); $mercado_pago = $this->mercado_pago;		
 		if($mercado_pago =='1'){
    ...
    if(empty($package['destination']['postcode'])){
      return;
        ...
        if($services['preciofijo'] > 1){
            $titulo = $services['service_name'];
            if($seguro > $this->ajuste_gratis){
              $titulo = $titulo . ' GRATIS!';
              $precio = 0;
            } else {
               $precio = $services['preciofijo'];
            }
          ...
          $params = array(
              "method" => array(
                   "get_rates" => array( 'api_user' => $this->api_user, 'api_password' => $this->api_password, 'api_confirmarretiro' => $this->api_confirmarretiro, 'api_key' => $this->api_key, 'api_nrocuenta' => $this->api_nrocuenta, 'operativa' => $services['operativa'], 'peso_total' => $andreani_weightb, 'volumen_total' => $andreani_volumesy, 		'cp_origen' => $this->origin, 'cp_destino' => $package['destination']['postcode'], 'n_paquetes' => $andreani_packageb, 'valor_declarado' => $seguro, )
              )
          );
         ...
         if ( !is_wp_error( $andreani_response ) ) {
          ...
          if (!empty($andreani_response->notice)) {
            if ( !isset($_COOKIE['andreani_notice'])) { setcookie( 'andreani_notice', $andreani_response->notice, time()+3600, "/"); } else { setcookie("andreani_notice", $andreani_response->notice, time()+3600, "/"); }
            return;
          } else { setcookie("andreani_notice", "", time() - 3600, "/");}
          //if(!empty($andreani_response->error)){ echo '<ul class="woocommerce-error"><li>'. $andreani_response->error .'</li> </ul>'; return; }				
          if($this->ajuste_precio == '0'){ $ajuste = '1'; } else if($this->ajuste_precio == '0%'){ $ajuste = '1'; } else { $ajuste = $this->ajuste_precio; }
          $redondear_total = $this->redondear_total; $porcentaje = $andreani_response->results * $ajuste / 100; $precio = $andreani_response->results + $porcentaje; $precio_base = $andreani_response->results + $porcentaje;
          if($redondear_total=='1'){ $precio = round($precio, 0, PHP_ROUND_HALF_UP); }
          if($mercado_pago =='1'){
            ...
            if($services['woocommerce_andreani_wanderlust_modalidad'] == 'sasp' || $services['woocommerce_andreani_wanderlust_modalidad'] == 'sapp' || $services['woocommerce_andreani_wanderlust_modalidad'] == 'pasp' || $services['woocommerce_andreani_wanderlust_modalidad'] == 'papp'){
              $titulo = $services['service_name'] . ': $' . $precio ; 	$precio = '0'; } else { $titulo = $services['service_name']; }
