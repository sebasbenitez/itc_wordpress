...
<?php do_action( 'woocommerce_email_header', $email_heading, $email ); ?>
	...
	<?php if ( $coupon ) : ?>
		...
		echo wp_kses(
			sprintf(
				/* translators: 1. Coupon value. 2. Coupon code. */
				__( 'Take %1$s OFF | Code: <strong>%2$s</strong>', 'woodmart' ),
				$coupon_value,
	...
	<table class="td xts-prod-table" cellspacing="0" cellpadding="6" border="1">
		...
		<tbody>
			<?php foreach ( $email->object->_cart->get_cart_contents() as $cart_id => $cart_item ) : ?>
				<?php
				$id      = isset( $cart_item['variation_id'] ) && ! empty( $cart_item['variation_id'] ) ? $cart_item['variation_id'] : $cart_item['product_id']; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
						...
						<a href="<?php echo esc_url( $product_url ); ?>">
							...
							<?php if ( $image ) : ?>
								<div style="margin-bottom: 5px">
									<?php echo wp_kses_post( apply_filters( 'woocommerce_order_item_thumbnail', $image ) ); ?>
								</div>
							<?php endif; ?>
						</a>
					</td>
			...
			foreach ( $item_totals as $item_id => $total ) {
				if ( 'est_del' === $item_id ) {
					continue;
				}
				?>
	...
	<div style="margin:0 0 16px;">
		<a class="xts-add-to-cart" href="<?php echo esc_url( $recover_button_link ); ?>">
			<?php echo apply_filters( 'woodmart_waitlist_label_confirm_button', __( 'Recover cart', 'woodmart' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</a>
	</div>
