...
class Shop_Archive extends Layout_Type {
	...
	public function check( $condition, $type = '' ) {
		...
		switch ( $condition['condition_type'] ) {
			...
			case 'product_term':
				...
				$taxonomy_type = is_object( $object ) && property_exists( $object, 'taxonomy' ) ? $object->taxonomy : false;

				$condition['condition_query'] = apply_filters( 'wpml_object_id', $condition['condition_query'], $taxonomy_type );
			...
			case 'product_attr':
				...
				$condition['condition_query'] = apply_filters( 'wpml_object_id', $condition['condition_query'], $taxonomy );
	...
	public function before_template_content() {
		if ( ! woodmart_is_woo_ajax() ) {
			parent::before_template_content();
		} else {
