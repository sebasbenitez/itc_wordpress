...
if ( ! function_exists( 'woodmart_get_vc_map_single_post_title' ) ) {
	/**
	 * Post title map.
	 */
	function woodmart_get_vc_map_single_post_title() {
		$typography = woodmart_get_typography_map(
			array(
				'key'      => 'typography',
				'selector' => '{{WRAPPER}} .wd-post-title.title',
			)
		);

		return array(
			'base'        => 'woodmart_single_post_title',
			'name'        => esc_html__( 'Post title', 'woodmart' ),
			'description' => esc_html__( 'Shows the post title', 'woodmart' ),
			'icon'        => WOODMART_ASSETS . '/images/vc-icon/post-icons/post-title.svg',
			...
			'params'      => array(
				...
				array(
					...
					'param_name'       => 'color',
					'selectors'        => array(
						'{{WRAPPER}} .wd-post-title.title' => array(
							'color: {{VALUE}};',
						),
					),
