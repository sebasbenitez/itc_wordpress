	...
	function woodmart_get_localized_string_array() {
		...
		return apply_filters(
			...
			array(
				...
				'breadcrumbs_separator'                  => woodmart_get_opt( 'breadcrumbs_separator', '/' ),
			)
		);
	}
}
...
if ( ! function_exists( 'woodmart_enqueue_base_styles' ) ) {
	...
	function woodmart_enqueue_base_styles() {
		...
		if ( defined( 'WPB_VC_VERSION' ) ) {
			if ( woodmart_get_opt( 'light_wpb_css' ) ) {
				wp_deregister_style( 'js_composer_front' );

				wp_register_style( 'js_composer_front', WOODMART_THEME_DIR . '/css/light-wpbakery.css', array(), $version );
				wp_style_add_data( 'js_composer_front', 'path', WOODMART_THEMEROOT . '/css/light-wpbakery.css' );
			}
		...
		// Frontend admin bar.
		if ( is_admin_bar_showing() ) {
			wp_enqueue_style( 'woodmart-frontend-admin-bar', WOODMART_ASSETS . '/css/parts/base-adminbar.css', array(), $version );
		}
		...
		if ( woodmart_is_elementor_installed() && function_exists( 'woodmart_elementor_is_edit_mode' ) && ( woodmart_elementor_is_edit_mode() || woodmart_elementor_is_preview_page() || woodmart_elementor_is_preview_mode() ) ) {
			wp_enqueue_style( 'woodmart-elementor-editor', WOODMART_THEME_DIR . '/inc/admin/assets/css/parts/int-elem-editor.css', array(), $version );
		}
...
if ( ! function_exists( 'woodmart_force_enqueue_styles' ) ) {
	...
	function woodmart_force_enqueue_styles() {
		...
		woodmart_force_enqueue_style( 'style-base-deferred' );
		...
		if ( is_admin_bar_showing() ) {
			woodmart_force_enqueue_style( 'admin-bar' );
		}

		if ( defined( 'WPSEO_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-yoast' );
		}

		if ( defined( 'RANK_MATH_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-rank-math' );
		}

		if ( defined( 'AIOSEO_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-aioseo' );
		}

		if ( defined( 'SEOPRESS_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-seopress' );
		}
		...
		if ( class_exists( 'bbPress' ) ) {
			woodmart_force_enqueue_style( 'bbpress' );
		}

		if ( class_exists( 'WOOCS_STARTER' ) ) {
			woodmart_force_enqueue_style( 'woo-curr-switch' );
		}

		if ( class_exists( 'WeDevs_Dokan' ) ) {
			woodmart_force_enqueue_style( 'woo-dokan-vend' );
		}

		$vendor_layout = function_exists( 'dokan_get_option' ) ? dokan_get_option( 'vendor_layout_style', 'dokan_appearance', 'legacy' ) : false;

		if ( 'latest' === $vendor_layout && function_exists( 'dokan_is_seller_dashboard' ) && dokan_is_seller_dashboard() ) {
			woodmart_force_enqueue_style( 'woo-dokan-backend' );
		}
		...
		if ( defined( 'WC_STRIPE_VERSION' ) && woodmart_woocommerce_installed() && ( is_product() || is_cart() || is_checkout() || is_account_page() ) ) {
			woodmart_force_enqueue_style( 'woo-stripe' );
		}

		if ( defined( 'WCPAY_PLUGIN_FILE' ) ) {
			woodmart_force_enqueue_style( 'woo-payments' );
		}

		if ( defined( 'KCO_WC_VERSION' ) || defined( 'WC_KLARNA_PAYMENTS_VERSION' ) ) {
			woodmart_force_enqueue_style( 'woo-klarna' );
		}

		if ( defined( 'PAYPAL_API_URL' ) ) {
			woodmart_force_enqueue_style( 'woo-paypal-payments' );
		}
		...
		if ( class_exists( 'WC_Dependencies_Product_Vendor' ) ) {
			woodmart_force_enqueue_style( 'woo-multivendorx' );
		}

		if ( class_exists( 'WC_Vendors' ) ) {
			woodmart_force_enqueue_style( 'woo-wc-vendors' );
		}

		if ( class_exists( 'WC_Subscriptions' ) ) {
			woodmart_force_enqueue_style( 'woo-mod-variation-form' );
			woodmart_force_enqueue_style( 'int-woo-subscriptions' );
		}

		if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
			woodmart_force_enqueue_style( 'wpml' );
		}

		if ( defined( 'WCML_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-wpml-curr-switch' );
		}

		if ( defined( 'YITH_WOOCOMPARE_VERSION' ) ) {
			woodmart_force_enqueue_style( 'woo-yith-compare' );
			woodmart_force_enqueue_style( 'colorbox-popup' );
		}

		if ( defined( 'YITH_WPV_VERSION' ) ) {
			woodmart_force_enqueue_style( 'woo-yith-vendor' );
		}

		if ( defined( 'YITH_YWRAQ_VERSION' ) ) {
			woodmart_force_enqueue_style( 'woo-yith-req-quote' );
		...
		if ( defined( 'YITH_WCWL' ) ) {
			woodmart_force_enqueue_style( 'woo-yith-wishlist' );
		...
		if ( defined( 'ELEMENTOR_PRO_VERSION' ) ) {
			woodmart_force_enqueue_style( 'elementor-pro-base' );
		}
		...
		if ( class_exists( 'PaymentPlugins\WooCommerce\PPCP\Main' ) ) {
			woodmart_force_enqueue_style( 'woo-payment-plugin-paypal' );
		}

		if ( defined( 'WC_STRIPE_PLUGIN_FILE_PATH' ) ) {
			woodmart_force_enqueue_style( 'woo-payment-plugin-stripe' );
		}

		if ( defined( 'WORDFENCE_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-wordfence' );
		}

		if ( defined( 'AMELIA_VERSION' ) ) {
			woodmart_force_enqueue_style( 'amelia' );
		}

		if ( defined( 'FPD_PLUGIN_DIR' ) ) {
			woodmart_force_enqueue_style( 'int-woo-fpd' );
		}

		if ( defined( 'VISA_ACCEPTANCE_PLUGIN_VERSION' ) ) {
			woodmart_force_enqueue_style( 'int-woo-vas' );
		}

		if ( defined( 'GREENSHIFT_DIR_URL' ) ) {
			woodmart_force_enqueue_style( 'int-greenshift' );
		}

		if ( woodmart_woocommerce_installed() ) {
			woodmart_force_enqueue_style( 'woocommerce-base' );
			woodmart_force_enqueue_style( 'woocommerce-base-deferred' );
			woodmart_force_enqueue_style( 'mod-notices-general' );
			...
			if ( woodmart_get_opt( 'show_sku_in_mini_cart' ) || woodmart_get_opt( 'show_sku_in_cart' ) || woodmart_get_opt( 'show_sku_in_checkout_page' ) || woodmart_get_opt( 'show_sku_in_thank_you_page' ) ) {
				woodmart_force_enqueue_style( 'woo-mod-small-prod-detail' );
			}
		}
...
if ( ! function_exists( 'woodmart_enqueue_inline_style' ) ) {
	 ...
	 */
	function woodmart_enqueue_inline_style( $key, $deferred = false ) {
		if (
			(
				function_exists( 'wc' ) &&
				(
		...
		Parts_Css_Files::get_instance()->enqueue_inline_style( $key, $deferred );
	}
}
...
if ( ! function_exists( 'woodmart_force_enqueue_style' ) ) {
	 ...
	 * @param string $key File slug.
	 */
	function woodmart_force_enqueue_style( $key ) {
		Parts_Css_Files::get_instance()->enqueue_style( $key );
	}
}
...
if ( ! function_exists( 'woodmart_enqueue_widgets_sidebar_style' ) ) {
	...
	function woodmart_enqueue_widgets_sidebar_style( $index ) {
		...
		foreach ( $sidebars_widgets[ $index ] as $widget ) {
			...
			} elseif ( str_starts_with( $widget, 'text-' ) ) {
				woodmart_enqueue_inline_style( 'widget-text' );
			}
		}
	}
...
if ( ! function_exists( 'woodmart_enqueue_style_in_iframe' ) ) {
	...
	function woodmart_enqueue_style_in_iframe() {
		...
		wp_enqueue_style( 'wd-widget-general' );

		wp_enqueue_style( 'wd-widgets-editor-style', WOODMART_STYLES . '/parts/wp-editor-widgets' . $rtl . '.css', array(), woodmart_get_theme_info( 'Version' ) );

		$storage       = new Styles_Storage( 'theme_settings_default' );
		$icons_storage = new Styles_Storage( 'icons_fonts' );

		$storage->print_styles();
		$icons_storage->print_styles();
	}
...
if ( ! function_exists( 'woodmart_enqueue_contact_form_style' ) ) {
	...
	function woodmart_enqueue_contact_form_style( $elements ) {
		...
		woodmart_enqueue_inline_style( 'wpcf7' );
