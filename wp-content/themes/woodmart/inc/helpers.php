...
if ( ! function_exists( 'woodmart_get_custom_labels_array' ) ) {
	/**
	 * Get custom labels array.
	 *
	 * @return array
	 */
	function woodmart_get_custom_labels_array() {
		$labels = array();
...
if ( ! function_exists( 'woodmart_get_current_theme_settings_fonts_labels' ) ) {
	/**
	 * Get current theme settings fonts labels.
	 *
	 * @param array|null $fonts_ids Optional array of font IDs to retrieve labels for.
	 * @return array
	 */
	function woodmart_get_current_theme_settings_fonts_labels( $fonts_ids = null ) {
		$inheritance_map = array(
			'text-font'          => '',
			'primary-font'       => 'text-font',
			'post-titles-font'   => 'primary-font',
			'secondary-font'     => 'text-font',
			'widget-titles-font' => 'primary-font',
			'navigation-font'    => 'text-font',
		);

		$fonts_labels = array(
			'text-font'          => esc_html__( 'Text font', 'woodmart' ),
			'primary-font'       => esc_html__( 'Title font', 'woodmart' ),
			'post-titles-font'   => esc_html__( 'Entities names font', 'woodmart' ),
			'secondary-font'     => esc_html__( 'Secondary font', 'woodmart' ),
			'widget-titles-font' => esc_html__( 'Widget titles font', 'woodmart' ),
			'navigation-font'    => esc_html__( 'Header font', 'woodmart' ),
		);

		$font_option_keys = array_keys( $fonts_labels );

		if ( is_array( $fonts_ids ) ) {
			$font_option_keys = array_values( array_intersect( $font_option_keys, $fonts_ids ) );
			$fonts_labels     = array_intersect_key( $fonts_labels, array_flip( $font_option_keys ) );
		}

		$fonts = array();

		foreach ( $font_option_keys as $font_key ) {
			$fonts[ $font_key ] = woodmart_get_opt( $font_key );
		}

		foreach ( $fonts as $key => $font ) {
			$font_family = isset( $font[0]['font-family'] ) ? $font[0]['font-family'] : '';

			if ( isset( $inheritance_map[ $key ] ) && ! empty( $inheritance_map[ $key ] ) ) {
				$parent_key         = $inheritance_map[ $key ];
				$parent_font_family = '';

				if ( isset( $fonts[ $parent_key ] ) && ! empty( $fonts[ $parent_key ] ) && isset( $fonts[ $parent_key ][0]['font-family'] ) && ! empty( $fonts[ $parent_key ][0]['font-family'] ) ) {
					$parent_font_family = $fonts[ $parent_key ][0]['font-family'];
				}
			}

			$is_css_var = str_contains( $font_family, 'var(--' );

			if ( ( empty( $font_family ) || $is_css_var ) && ! empty( $parent_font_family ) ) {
				$fonts_labels[ $key ] .= ' (' . $parent_font_family . ')';
			} elseif ( ! empty( $font_family ) && ! $is_css_var ) {
				$fonts_labels[ $key ] .= ' (' . $font_family . ')';
			}
		}
