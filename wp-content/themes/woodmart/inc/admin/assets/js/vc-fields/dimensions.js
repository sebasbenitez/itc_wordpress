(function($) {
	$('#vc_ui-panel-edit-element').on('vcPanel.shown', function() {
		$('.wd-dimensions-wrapp').each(function() {
			...
			$wrapper.find('.wd-dimensions').each(function() {
				...
				$this.find('.wd-dimension-unit-control').on( 'click', function() {
					...
					setInputsType($this.parents('.wd-dimensions'));
		 ...
		 * Set inputs type.
		 */
		function setInputsType($this) {
			let unit = $this.data('unit')
			let inputType = 'custom' === unit ? 'text' : 'number'
			$this.find('.wd-dimension-field-value-display').attr('type', inputType)
		}
	});
})(jQuery);
