...
abstract class Field {
	 ...
	 * @param string $field_object Object for post or term.
	 */
	public function __construct( $args, $options, $type = 'options', $field_object = 'post' ) {
		$this->args = $args;
		$this->id   = $args['id'];
	 ...
	 * @since 1.0.0
	 *
	 * @param object $instance Post object for metaboxes fields.
	 */
	public function set_post( $instance ) {
		if ( 'metabox' === $this->type ) {
			if ( is_a( $instance, 'WP_Post' ) ) {
				$this->post = $instance;
			} elseif ( is_a( $instance, 'WP_Term' ) ) {
				$this->term = $instance;
			}
		}
	}
	 ...
	 * @since 1.0.0
	 *
	 * @param object $instance Post or Term object for metaboxes fields.
	 * @param bool   $preset Current field preset ID.
	 * @param bool   $is_inner_control Is this field is inner control for another field, for example for tabs.
	 *
	 * @return void
	 */
	public function render( $instance = null, $preset = false, $is_inner_control = false ) {
		if ( $preset ) {
			...
			<div class="xts-field xts-settings-field <?php echo esc_attr( $this->extra_css_class ); ?>" <?php $this->get_dependency_data_attribute(); ?>>
				<?php if ( ( isset( $this->args['description'] ) && ! empty( $this->args['description'] ) ) || ( isset( $this->args['name'] ) && ! empty( $this->args['name'] ) ) ) : ?>
					<div class="xts-option-title">
						<label>
							<?php if ( false !== $this->presets && 'notice' !== $this->args['type'] && ! isset( $this->args['tabs'] ) && isset( $_GET['preset'] ) && ! $is_inner_control ) : // phpcs:ignore ?>
	 ...
	 *
	 * @param array $requires Dependency array.
	 *
	 * @since 1.0.0
	 */
			...
			switch ( $dependency['compare'] ) {
				case 'equals':
					if ( ! is_null( $parent_value ) ) {
						if ( is_array( $dependency['value'] ) ) {
							$shown = in_array( $parent_value, $dependency['value'] ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
						} else {
							$shown = $parent_value == $dependency['value']; // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
						}
					}
					break;
				case 'not_equals':
					if ( ! is_null( $parent_value ) ) {
						if ( is_array( $dependency['value'] ) ) {
							...
							$shown = $parent_value != $dependency['value']; // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
						}
					}
					break;
			}
	 ...
	 * @param bool $subkey4 Subkey for array fields. Fourth level.
	 *
	...
	public function get_field_value( $subkey = false ) {
		...
		$object = $this->post ? $this->post : $this->term;

		if ( 'metabox' === $this->type && ! is_null( $object ) ) {
			$object_id = $this->post ? $this->post->ID : $this->term->term_id;
			$val       = get_metadata( $this->object, $object_id, $this->get_input_name(), true );
		} elseif ( false !== $this->presets ) {
			foreach ( $this->presets as $preset_id ) {
				if ( isset( $this->options[ $preset_id ] ) && isset( $this->options[ $preset_id ][ $this->args['id'] ] ) ) {
					$val = $this->options[ $preset_id ][ $this->args['id'] ];
				}
			}

			if ( isset( $this->options[ $this->args['id'] ] ) && empty( $val ) && '0' !== $val ) {
		...
		if ( 'metabox' === $this->type && empty( $val ) && '0' !== $val ) {
	 ...
	 *
	 * @return string Default value for the field.
	 */
	...
	/**
	 * Check if field requires to generate CSS code based on the dependencies.
	 */
