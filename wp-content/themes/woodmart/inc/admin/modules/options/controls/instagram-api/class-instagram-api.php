...
class Instagram_Api extends Field {
	...
	public function render_control() {
		if ( ! empty( $_GET['instagram_account_id'] ) && ! empty( $_GET['instagram_access_token'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			echo '<div class="xts-notice xts-success">Access token generated</div>';
			$this->connect();
		}

		if ( isset( $_GET['instagram_account_disconnect'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			$this->disconnect();
		}
		...
		?>
			<div class="xts-upload-btns">
				...
				<?php if ( $instagram_access_token ) : ?>
					<a href="<?php echo esc_url( $this->get_return_url() . '&instagram_account_disconnect' ); ?>" class="xts-btn xts-color-warning xts-remove-upload-btn xts-i-trash">
	...
	public function connect() {
		update_option( 'instagram_account_id', sanitize_text_field( $_GET['instagram_account_id'] ) ); //phpcs:ignore WordPress.Security
		update_option( 'instagram_access_token', sanitize_text_field( $_GET['instagram_access_token'] ) ); //phpcs:ignore WordPress.Security
	}
	...
	/**
	 * Get return url after instagram auth.
	 *
	 * @return string
	 */
	...
	public function get_link() {
		...
		$redirect_uri  = urlencode( 'https://xtemos.com/instagram.php' ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
		$response_type = 'code';
		...
		$return_url    = urlencode( $this->get_return_url() ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions

		return $base_url . '?response_type=' . $response_type . '&client_id=' . $app_id . '&redirect_uri=' . $redirect_uri . '&scope=' . $scope . '&state=' . $return_url;
	}

	/**
	 * Display connected account data.
	 */
	public function show_connected_account() {
		...
		<div class="xts-instagram-profile">
			<div class="xts-instagram-pic"><img src="<?php echo esc_url( $instagram_account_data['image'] ); ?>" alt=""></div>
			<div class="xts-instagram-name"><?php echo esc_html( $instagram_account_data['name'] ); ?> : <span>@<?php echo esc_html( $instagram_account_data['username'] ); ?></span></div>
		</div>
		<?php
	}

	/**
	 * Get connected account data from instagram API.
	 *
	 * @return array|void
	 */
	public function get_connected_account_data() {
		...
		if ( is_wp_error( $account_data ) ) {
			echo 'Get connected account data :' . esc_html( $account_data->get_error_message() );
			return;
		}
		...
		if ( is_object( $data_decoded ) && property_exists( $data_decoded, 'error' ) ) {
			echo 'Get connected account data :' . esc_html( $data_decoded->error->message );
			return;
		}
