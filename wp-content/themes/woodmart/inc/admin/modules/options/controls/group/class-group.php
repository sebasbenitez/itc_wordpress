...
class Group extends Field {
	...
	private function render_inner_fields() {
		foreach ( $this->inner_fields as $field ) {
			if ( 'metabox' === $this->type ) {
				if ( $this->post ) {
					$field->render( $this->post );
				} elseif ( $this->term ) {
					$field->render( $this->term );
				}
			} else {
	...
	public function css_output() {
		...
		$is_active_preset = 'metabox' !== $this->type && Registry::get_instance()->themesettingscss->is_preset_active();
		$output_css       = array(
			$device => array(),
		);
		...
		foreach ( $this->inner_fields as $inner_field ) {
			if ( 'metabox' === $this->type ) {
				if ( $this->term ) {
					$inner_field->set_post( $this->term );
				} else {
					$inner_field->set_post( $this->post );
				}
			} else {
				$inner_field->set_presets( $this->presets );
			}

			if ( 'metabox' === $this->type || ! $is_active_preset || woodmart_is_opt_changed( $inner_field->args['id'] ) ) {
				if ( 'metabox' === $this->type && $this->options ) {
					$object_id = $this->post ? $this->post->ID : $this->term->term_id;

					foreach ( $this->options as $key => $value ) {
						if ( ! $value ) {
							$this->options[ $key ] = get_metadata( $this->object, $object_id, $key, true );
						}
					}
