...
if ( ! function_exists( 'woodmart_get_products_shortcode_map_params' ) ) {
	/**
	 * Get params for products shortcode map.
	 *
	 * @return array
	 */
...
if ( ! function_exists( 'woodmart_get_products_shortcode_params' ) ) {
	/**
	 * Get params for products shortcode.
	 *
	 * @return array
	 */
	function woodmart_get_products_shortcode_params() {
		...
		$post_type_array            = array(
			...
			esc_html__( 'New products', 'woodmart' )       => 'new',
		...
		return apply_filters(
			'woodmart_get_products_shortcode_params',
			array(
				array(
				...
				*/
				...
				array(
					'type'             => 'dropdown',
					'param_name'       => 'list_spacing_tablet',
					'value'            => array(
						esc_html__( 'Inherit from Theme Settings', 'woodmart' ) => '',
						30 => 30,
						20 => 20,
				...
				array(
					'type'             => 'dropdown',
					'param_name'       => 'list_spacing_mobile',
					'value'            => array(
						esc_html__( 'Inherit from Theme Settings', 'woodmart' ) => '',
						30 => 30,
						20 => 20,
				...
				*/
				...
				array(
					...
					'extra_content' => function_exists( 'woodmart_get_html_block_links' ) ? woodmart_get_html_block_links( 'edit.php?post_type=woodmart_layout&wd_layout_type_tab=loop_item&create_template', __( 'layout', 'woodmart' ) ) : '',
					'dependency'    => array(
				...
				array(
					'type'             => 'textfield',
					'heading'          => esc_html__( 'Images resolution', 'woodmart' ),
					'group'            => esc_html__( 'Design', 'woodmart' ),
					'param_name'       => 'img_size',
				...
				array(
					...
					'dependency'       => array(
						'element'            => 'product_hover',
						'value_not_equal_to' => array( 'small', 'custom' ),
					),
					'edit_field_class' => 'vc_col-sm-6 vc_column',
				),
				array(
...
add_filter( 'vc_autocomplete_woodmart_products_include_callback', 'woodmart_product_id_autocomplete_suggester_new', 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_woodmart_products_include_render', 'woodmart_product_id_autocomplete_render', 10, 1 ); // Render exact product. Must return an array (label,value)
...
if ( ! function_exists( 'woodmart_vc_autocomplete_taxonomies_field_render' ) ) {
	/**
	 * Render exact term for autocomplete taxonomy field.
	 *
	...
	function woodmart_vc_autocomplete_taxonomies_field_render( $term ) {
		...
		$terms = get_terms(
			array(
				'taxonomy'   => array_keys( $vc_taxonomies_types ),
...
if ( ! function_exists( 'woodmart_vc_autocomplete_taxonomies_field_search' ) ) {
	/**
	 * Narrow data taxonomies for autocomplete taxonomy field.
	 *
