...
class Frontend extends Singleton {
	...
	public function update_frequently_bought_price() {
		...
		add_filter( 'woocommerce_product_get_sale_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10, 2 );
		add_filter( 'woocommerce_product_get_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10, 2 );
		add_filter( 'woocommerce_product_variation_get_sale_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10, 2 );
		add_filter( 'woocommerce_product_variation_get_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10, 2 );
		add_filter( 'woocommerce_variation_prices', array( $this, 'set_fbt_dynamic_variation_prices' ), 10, 3 );
		...
		if ( $products_id ) {
			foreach ( $products_id as $id => $variation_id ) {
				...
				if ( $variation_id ) {
					...
					$fragments[ 'div.wd-fbt-bundle-' . $this->bundle_id . ' .wd-product-' . $id . ' .price' ] = '<span class="price">' . $variation_product->get_price_html() . '</span>';
				} else {
					$current_product = wc_get_product( $id );
					$current_product->get_price_html();
				}
			}
		}

		remove_filter( 'woocommerce_product_get_sale_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10 );
		remove_filter( 'woocommerce_product_get_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10 );
		remove_filter( 'woocommerce_product_variation_get_sale_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10 );
		remove_filter( 'woocommerce_product_variation_get_price', array( $this, 'set_fbt_dynamic_sale_price' ), 10 );
		remove_filter( 'woocommerce_variation_prices', array( $this, 'set_fbt_dynamic_variation_prices' ), 10 );
	...
	/**
	 * Set dynamic sale price and calculate subtotal.
	 *
	 * @param string $price Product price.
	 * @param object $product Product data.
	 * @return float
	 */
	public function set_fbt_dynamic_sale_price( $price, $product ) {
		...
		$new_price = $price;
		if ( $discount && $discount <= 100 && $old_price ) {
			$new_price = (float) ( $old_price - ( ( $old_price / 100 ) * $discount ) );
		}

		if ( isset( $this->subtotal_products_price[ $product_id ]['old'] ) ) {
			return $new_price;
		}

		$this->subtotal_products_price[ $product_id ]['old'] = 0;
		$this->subtotal_products_price[ $product_id ]['new'] = 0;

		$old_price_with_tax = (float) wc_get_price_to_display( $product, array( 'price' => $old_price ) );

		if ( ! $discount || 100 < $discount ) {
			if ( $product->is_on_sale() ) {
				$this->subtotal_products_price[ $product_id ]['old'] = (float) wc_get_price_to_display( $product, array( 'price' => (float) $product->get_regular_price( 'edit' ) ) );
			} else {
		...
		$new_price_with_tax = (float) wc_get_price_to_display( $product, array( 'price' => $new_price ) );
	...
	/**
	 * Set dynamic variation prices.
	 *
	 * @param array  $prices_array Associative array of variation prices indexed by variation ID.
	 * @param object $product The variable product object.
	 * @param bool   $for_display Whether prices are being retrieved for display.
	 * @return array
	 */
	public function set_fbt_dynamic_variation_prices( $prices_array, $product, $for_display ) {
		$product_id = $product->get_ID();
		...
		if ( isset( $prices_array['price'] ) && is_array( $prices_array['price'] ) ) {
			foreach ( $prices_array['price'] as $variation_id => $old_price ) {
				$old_price = (float) $old_price;

				if ( $old_price ) {
					$new_price = (float) ( $old_price - ( ( $old_price / 100 ) * $discount ) );

					$prices_array['price'][ $variation_id ]      = $new_price;
					$prices_array['sale_price'][ $variation_id ] = $new_price;
				}
			}
		}
	 ...
	 * @return false|float
	 */
	public function get_discount_product_bundle( $product_id ) {
