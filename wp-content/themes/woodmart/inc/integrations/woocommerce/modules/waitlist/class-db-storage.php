...
class DB_Storage extends Singleton {
	...
	/**
	 * Waitlist cache version option name.
	 *
	 * @const string
	 */
	const CACHE_VERSION_OPTION = 'wd_waitlist_cache_version';
	...
	public function create_subscription( $email, $product, $email_language = '' ) {
		...
		if ( $wpdb->insert_id ) {
			$this->bump_waitlist_cache_version();
		}
	...
	public function unsubscribe_by_token( $token ) {
		...
		$deleted = $wpdb->delete( $wpdb->wd_waitlists, array( 'unsubscribe_token' => $token ) ); // phpcs:ignore.
	...
	public function unsubscribe_current_user( $product ) {
		...
		$deleted = $wpdb->delete( $wpdb->wd_waitlists, $data ); // phpcs:ignore.
	...
	public function unsubscribe_by_product( $product ) {
		...
		$deleted = $wpdb->delete( $wpdb->wd_waitlists, $this->get_product_ids_by_type( $product ) ); // phpcs:ignore.
	...
	/**
	 * Increments waitlist cache version to invalidate list-table cache keys.
	 *
	 * Only updates the version when a persistent object cache (Redis, Memcached, etc.) is available.
	 * In default in-memory cache, cache doesn't persist across requests anyway, so bumping is unnecessary.
	 *
	 * @return void
	 */
	private function bump_waitlist_cache_version() {
		// Skip version bump if no persistent cache is available to avoid unnecessary DB queries.
