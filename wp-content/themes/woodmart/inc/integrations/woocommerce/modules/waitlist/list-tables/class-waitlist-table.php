...
class Waitlist_Table extends WP_List_Table {
	...
	/**
	 * Prints column for waitlist product thumbnail.
	 *
	 * @param array $item Item to use to print record.
	 * @return string
	 */
	public function column_thumb( $item ) {
		...
		?>
		<a href="<?php echo esc_url( strval( get_edit_post_link( $product->get_id() ) ) ); ?>">
	 ...
	 * @param string|int|false $product_id Product ID filter.
	 * @param string|int|false $user_id User ID filter.
	 * @param string|false     $user_email User email filter.
	 *
	 * @return string
	 */
	private function get_table_data_cache_key( $search, $product_id, $user_id, $user_email ) {
		$cache_version = (int) get_option( DB_Storage::CACHE_VERSION_OPTION, 1 );

		return 'wd_waitlist_table_data_' . md5( wp_json_encode( array( (string) $search, (int) $product_id, (int) $user_id, (string) $user_email, $cache_version ) ) );
	}
