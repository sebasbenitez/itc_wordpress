...
class Main extends Singleton {
	...
	public function add_options() {
		...
		Options::add_field(
			array(
				...
				'default'     => '0',
				'tags'        => array(
					esc_html__( 'live visitors', 'woodmart' ),
					esc_html__( 'social proof', 'woodmart' ),
					esc_html__( 'popularity', 'woodmart' ),
					esc_html__( 'product views', 'woodmart' ),
				),
				'priority'    => 10,
			)
		);
	 ...
	 * @param string $icon_output Icon output.
	 */
	...
	public function get_count_visits_in_db( $product_id, $live_visits = 0 ) {
		...
		if ( 'fake_data' === woodmart_get_opt( 'counter_visitor_data_source' ) ) {
			...
			$last_product_visits_key = array_key_last( $product_visits );

			if (
				! empty( $last_product_visits_key ) &&
				! empty( $product_visits[ $last_product_visits_key ] ) &&
				$product_visits[ $last_product_visits_key ] + $time_limited < $current_time
			) {
				$product_visits = array();
			}
