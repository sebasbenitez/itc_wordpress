...
if ( ! function_exists( 'woodmart_sguide_table_template' ) ) {
	/**
	 * Size guide table template
	 *
	 * @param array $tables Size guide tables.
	 */
	function woodmart_sguide_table_template( $tables ) {
		...
		wp_enqueue_style( 'woodmart-edittable-style', WOODMART_ASSETS . '/css/jquery.edittable.css', array(), WOODMART_VERSION );
		wp_enqueue_style( 'wd-admin-page-size-guides', WOODMART_ASSETS . '/css/parts/page-size-guides.css', array(), WOODMART_VERSION );
		?>
		<textarea class="woodmart-sguide-table-edit" name="woodmart-sguide-table" style="display:none;">
			<?php echo wp_json_encode( $tables ); ?>
		</textarea>
		<?php
	}
}
...
if ( ! function_exists( 'woodmart_sguide_table_save' ) ) {
	/**
	 * Size guide table save
	 *
	 * @param int $post_id Current post ID.
	 */
	function woodmart_sguide_table_save( $post_id ) {
		if ( ! isset( $_POST['woodmart-sguide-table'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
			return;
		}

		$size_guide = json_decode( stripslashes( $_POST['woodmart-sguide-table'] ) );  // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
if ( ! function_exists( 'woodmart_sguide_dropdown_save' ) ) {
	/**
	 * Size guide dropdown save
	 *
	 * @param int $post_id Current post ID.
	 */
	function woodmart_sguide_dropdown_save( $post_id ) {
		if ( isset( $_POST['woodmart_sguide_select'] ) && $_POST['woodmart_sguide_select'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if ( isset( $_POST['woodmart_disable_sguide'] ) && 'on' === $_POST['woodmart_disable_sguide'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				update_post_meta( $post_id, 'woodmart_sguide_select', 'disable' );
			} else {
				update_post_meta( $post_id, 'woodmart_sguide_select', sanitize_text_field( $_POST['woodmart_sguide_select'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			}
		}
	}
}
...
if ( ! function_exists( 'woodmart_sguide_display' ) ) {
	...
	function woodmart_sguide_display( $post_id = false, $args = array() ) {
		...
		$sguide_post_id = get_post_meta( $post_id, 'woodmart_sguide_select', true );

		if ( ! empty( $sguide_post_id ) && 'disable' === $sguide_post_id ) {
			return;
		}

		if ( ! empty( $sguide_post_id ) && 'none' !== $sguide_post_id ) {
			$sguide_post_id = $sguide_post_id;
		} else {
			$sguide_post_id = '';
...
if ( ! function_exists( 'woodmart_sguide_display_table_template' ) ) {
	/**
	 * Render size guide popup markup (content and optional table) for the provided size guide post.
	 *
	 * @param WP_Post $sguide_post  Size guide post object.
	 * @param array   $size_tables  Size guide tables data.
	 * @param array   $args         Additional arguments.
	 *
	 * @return void
	 */
	function woodmart_sguide_display_table_template( $sguide_post, $size_tables, $args = array() ) {
		...
		$show_table = get_post_meta( $sguide_post->ID, 'woodmart_sguide_hide_table', true );
		$show_table = $show_table ? $show_table : 'show';
		...
		if ( woodmart_is_gutenberg_blocks_enabled() && has_blocks( $sguide_post->post_content ) ) {
			echo Blocks_Assets::get_instance()->get_inline_scripts( $sguide_post->ID ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo Post_CSS::get_instance()->get_inline_blocks_css( $sguide_post->ID ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		?>
			<?php if ( $wpb_shortcodes_custom_css || $woodmart_shortcodes_custom_css ) : ?>
				<style data-type="vc_shortcodes-custom-css">
					<?php echo $wpb_shortcodes_custom_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<?php echo $woodmart_shortcodes_custom_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				/* */
				</style>
			<?php endif; ?>
			<div id="wd_sizeguide" data-wrap-class="wd-sizeguide-wrap" class="mfp-hide wd-popup wd-popup-element wd-deferred wd-sizeguide wd-scroll-content">
			...
			<div class="wd-sizeguide-btn wd-action-btn wd-sizeguide-icon<?php echo esc_attr( $args['builder_classes'] ); ?>">
				<a class="wd-open-popup" rel="nofollow" href="#wd_sizeguide">
					...
					<span class="wd-action-text">
						<?php esc_html_e( 'Size guide', 'woodmart' ); ?>
					</span>
				</a>
			</div>
		<?php
...
if ( ! function_exists( 'woodmart_sguide_save_category' ) ) {
	...
	function woodmart_sguide_save_category( $post_id ) {
		if ( isset( $_POST['woodmart_sguide_category'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			$selected_sguide_category = woodmart_clean( $_POST['woodmart_sguide_category'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			foreach ( $terms as $term ) {
				if ( ! in_array( absint( $term->term_id ), array_map( 'absint', $selected_sguide_category ), true ) ) {
					if ( get_term_meta( $term->term_id, 'woodmart_chosen_sguide', true ) === $post_id ) {
...
if ( ! function_exists( 'woodmart_sguide_category_template' ) ) {
	...
	function woodmart_sguide_category_template( $post ) {
		...
		$chosen_cats     = get_post_meta( $post->ID, 'woodmart_chosen_cats', true );
		...
		<ul>
			<?php foreach ( $sguide_cat_list as $sguide_cat ) : ?>
				...
				if ( is_array( $chosen_cats ) && in_array( absint( $sguide_cat->term_id ), array_map( 'absint', $chosen_cats ), true ) ) {
					$checked = 'checked';
				}
				?>
				<li>
...
if ( ! function_exists( 'woodmart_sguide_hide_table_template' ) ) {
	...
	function woodmart_sguide_hide_table_template( $post ) {
		$disable_table = get_post_meta( $post->ID, 'woodmart_sguide_hide_table', true );
		$disable_table = $disable_table ? $disable_table : 'show';
		?>
		<label>
...
if ( ! function_exists( 'woodmart_sguide_hide_table_save' ) ) {
	...
	function woodmart_sguide_hide_table_save( $post_id ) {
		if ( isset( $_POST['woodmart_sguide_hide_table'] ) && 'on' === $_POST['woodmart_sguide_hide_table'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
