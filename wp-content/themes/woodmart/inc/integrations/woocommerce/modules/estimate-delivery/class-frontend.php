...
class Frontend extends Singleton {
	...
	public function enqueue_scripts() {
		...
		if (
			...
			woodmart_force_enqueue_style( 'woo-mod-product-info' );
			woodmart_force_enqueue_style( 'woo-opt-est-del' );
		}

		if (
			( woodmart_get_opt( 'estimate_delivery_show_on_single_product' ) && is_product() ) ||
			( woodmart_get_opt( 'estimate_delivery_show_on_cart_page' ) && is_cart() ) ||
			( woodmart_get_opt( 'estimate_delivery_show_on_checkout_page' ) && is_checkout() ) ||
			( woodmart_get_opt( 'estimate_delivery_show_on_order_details' ) && ( is_wc_endpoint_url( 'view-order' ) || is_wc_endpoint_url( 'order-received' ) || is_wc_endpoint_url( 'order-pay' ) ) ) ||
			woodmart_get_opt( 'estimate_delivery_show_on_mini_cart' )
		) {
	...
	public function enqueue_admin_scripts() {
		...
		if ( isset( $_GET['page'] ) && 'wc-orders' === $_GET['page'] ) {  // phpcs:ignore WordPress.Security
			wp_enqueue_style(
				...
				WOODMART_ASSETS . '/css/parts/int-woo-page-orders.css',
				array(),
