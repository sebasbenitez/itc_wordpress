...
class Main extends Singleton {
	...
	public function init() {
		...
		add_action( 'woocommerce_before_variations_form', array( $this, 'enqueue_variations_form_style' ), 10, 2 );
	}
	...
	public function output_unit_of_measure( $price, $wc_product ) {
		...
		if ( ! is_singular() || ! in_array( $wc_product->get_type(), array( 'variable', 'variation' ), true ) ) {
			woodmart_enqueue_inline_style( 'woo-mod-price-unit' );
		}

		?>
			<span class="wd-price-unit"><?php echo wp_kses( $unit_of_measure, true ); ?></span>
		<?php

		$content = ob_get_clean();

		return $price . $content . $wc_product->get_price_suffix();
	}
	...
	/**
	 * Enqueue styles for variations form.
	 *
	 * @return void
	 */
	public function enqueue_variations_form_style() {
		global $product;
		$unit_of_measure = $this->get_unit_of_measure_db( $product );
