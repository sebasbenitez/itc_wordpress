...
if ( ! class_exists( 'XTS_Email_Wishlist_Promotional' ) ) :
	...
	class XTS_Email_Wishlist_Promotional extends WC_Email {
		 ...
		 *
		 * @param array $email_class Email class.
		 * @return array
		 */
		...
		public function get_custom_content_html() {
			...
			foreach ( $this->items as $id => $product ) {
				...
				$this->placeholders = array_merge(
					...
					array_merge(
						array(
							'{product_image_' . $id . '}' => $product ? apply_filters( 'woodmart_promotional_email_item_thumbnail', '<div style="margin-bottom: 5px">' . $image . '</div>', $product ) : '',
		 ...
		 * Format email string.
		 *
		 * @param mixed $str Text to replace placeholders in.
		 * @return string
		 */
		public function format_string( $str ) {
			...
			if ( ! strpos( $str, $start ) || ! strpos( $str, $end ) ) {
				return parent::format_string( $str );
			}

			$start_pos           = strpos( $str, $start ) + strlen( $start );
			$end_pos             = strpos( $str, $end, $start_pos );
			$default_loop_string = substr( $str, $start_pos, $end_pos - $start_pos );
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'content_html' => array(
					...
					'type'        => 'textarea',
					'description' => sprintf(
						// translators: 1. Placeholders. 2. Loop tags. 3. Loop placeholders.
		...
		private function get_available_coupon_codes() {
			...
			$coupon_codes = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				"SELECT post_name
				FROM $wpdb->posts
		 ...
		 * @param string $placeholder_type Placeholder type.
		 *
