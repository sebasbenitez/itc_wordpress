...
class Manager extends Singleton {
	...
	public function check_free_gifts_totals( $gift_rule, $cart_price = false ) {
		if ( false === $cart_price ) {
			...
			if ( isset( $gift_rule['free_gifts_cart_price_type'] ) ) {
				switch ( $gift_rule['free_gifts_cart_price_type'] ) {
					case 'subtotal':
						$cart_price = $totals['subtotal'];
						break;
					case 'subtotal_with_taxes':
						$cart_price = $totals['subtotal'] + $totals['subtotal_tax'];
						break;
			...
			$cart_price = apply_filters( 'woodmart_free_gift_cart_price', $cart_price, $gift_rule, $totals );
		}

		$min_price = apply_filters( 'woodmart_free_gift_price', $gift_rule['free_gifts_cart_total_min'] );
		$max_price = apply_filters( 'woodmart_free_gift_price', $gift_rule['free_gifts_cart_total_max'] );
