...
if ( ! function_exists( 'woodmart_elementor_instagram_template' ) ) {
	/**
	 * Elementor template function for Instagram element.
	 *
	 * @param array $settings The settings for the Instagram element.
	 *
	 * @return void
	 */
	function woodmart_elementor_instagram_template( $settings ) {
		...
		?>
		<div id="<?php echo esc_attr( $carousel_id ); ?>" class="wd-insta<?php echo esc_attr( $wrapper_classes ); ?>">
			<?php if ( $settings['username'] && ! is_wp_error( $media_array ) ) : ?>
				...
				<div class="<?php echo esc_attr( $pics_classes ); ?>" <?php echo $carousel_atts; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					...
					<?php foreach ( $media_array as $item ) : ?>
						...
						if ( ! empty( $settings['size'] ) && ! empty( $item[ $settings['size'] ] ) ) {
						...
						<div class="wd-insta-item<?php echo esc_attr( $picture_classes ); ?>">
							...
							<?php if ( 0 === (int) $settings['hide_mask'] ) : ?>
			...
			<?php if ( $settings['link'] ) : ?>
					<a href="//www.instagram.com/<?php echo trim( $settings['username'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="wd-insta-link" rel="me" target="<?php echo esc_attr( $settings['target'] ); ?>"><?php echo esc_html( $settings['link'] ); ?></a>
			<?php endif; ?>
		</div>
		<?php
	}
